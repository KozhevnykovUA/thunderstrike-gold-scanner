...
for seg in SEGMENTS:
    ...
    for name,crafts,mats,recipe in seg['candidates']:
        out=by.get(name.lower()); sell=price(out)
        if sell is None: continue
        mat_cost=0; ok=True; details=[]
        for mat,qty in mats.items():
            p=price(by.get(mat.lower()))
            if p is None: ok=False; break
            mat_cost+=p*qty; details.append({'name':mat,'qty':qty,'unit_gold':round(p,4)})
        if not ok: continue
        rate,sold=metrics(s,int(out['itemId']))
        # One-listing expected recovery; conservative for slow items.
        recovery=sell*(1-AH_CUT)*(rate if rate is not None else 0.10)
        net=max(0,mat_cost-recovery)
        ranked.append({'output':name,'expected_crafts':crafts,'materials':details,'gross_per_craft_gold':round(mat_cost,4),'sell_gold':round(sell,4),'sale_rate':rate,'sold_per_day':sold,'expected_recovery_per_craft_gold':round(recovery,4),'net_per_craft_gold':round(net,4),'segment_net_gold':round(net*crafts,2),'recipe':recipe})
...
# AH prices for recipes/books when listed.
recipe_rows=[]
