...
for seg in SEGMENTS:
    ...
    for name,crafts,mats,recipe in seg['candidates']:
        ...
        for mat,qty in mats.items():
            matrow=lookup(mat,MAT_IDS); p=price(matrow)
        ...
        anomaly = sell > max(1.0, mat_cost * 20)
        recovery = 0 if anomaly else sell*(1-AH_CUT)*(rate if rate is not None else 0.10)
        net=max(0,mat_cost-recovery)
        ranked.append({'output':name,'output_item_id':int(out['itemId']),'expected_crafts':crafts,'materials':details,'gross_per_craft_gold':round(mat_cost,4),'sell_gold':round(sell,4),'sale_rate':rate,'sold_per_day':sold,'sale_price_anomaly':anomaly,'expected_recovery_per_craft_gold':round(recovery,4),'net_per_craft_gold':round(net,4),'segment_net_gold':round(net*crafts,2),'recipe':recipe})
